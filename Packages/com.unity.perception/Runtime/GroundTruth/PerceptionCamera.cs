...
namespace UnityEngine.Perception.GroundTruth
{
    ...
    public class PerceptionCamera : MonoBehaviour
    {
        ...
        public event Action<
            int,
            NativeArray<RenderedObjectInfo>,
        ...
        /// </summary>
        [Tooltip(
            "The minimum level of transparency that will be rendered per pixel in segmentation images."
        )]
        [Range(0f, 1f)]
        ...
        public void RequestCapture()
        {
            ...
            {
                Debug.LogError(
                    $"{nameof(RequestCapture)} can only be used if the camera is in "
        ...
        public T EnableChannel<T>()
            where T : CameraChannelBase, new()
        {
            if (m_BegunCapturingData)
                throw new InvalidOperationException(
        ...
        public bool IsChannelEnabled<T>()
            where T : CameraChannelBase
        {
        ...
        public bool TryGetChannel<T>(out T channel)
            where T : CameraChannelBase
        {
        ...
        internal void CheckFixedLengthScenarioHasEnoughFPI()
        {
            if (captureTriggerMode == CaptureTriggerMode.Scheduled)
            {
                ...
                if (scenario)
                {
                    ...
                    {
                        Debug.LogError(
                            $"The number of frames per Iteration of the {nameof(FixedLengthScenario)} on the object \"{scenario.name}\" is not large "
        ...
        void EnsureSensorRegistered()
        {
            ...
            {
                m_SensorDefinition = new RgbSensorDefinition(
                    id,
                    captureTriggerMode,
                    description,
                    firstCaptureFrame,
        ...
        void SetupVisualizationCamera()
        {
            ...
            {
                Debug.LogWarning(
                    "Currently only one PerceptionCamera may be visualized at a time. "
        ...
        void DisplayNoLabelersMessage()
        {
            ...
            GUILayout.Label(
                "Visualization: No labelers are currently active. Enable at least one labeler from the "
        ...
        static void CaptureHierarchy()
        {
            ...
            if (!savedHierarchies.ContainsKey(Time.frameCount))
                savedHierarchies[Time.frameCount] =
                    EnvironmentUtilities.ParseHierarchyFromAllScenes();
        ...
        void OnEndFrameRendering(ScriptableRenderContext ctx)
        {
            ...
            if (RenderedObjectInfosCalculated != null)
            {
                ...
                RenderedObjectInfoComputer.CalculateRenderedObjectInfos(
                    cmd,
                    texture,
                    RenderedObjectInfosCalculated
                );
        ...
        {
            k_RgbImageEncodingFormat = jpgExport
                ? ImageEncodingFormat.Jpg
