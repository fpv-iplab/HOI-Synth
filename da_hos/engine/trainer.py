...
from da_hos.data.dataset_mapper import (
    DatasetMapperTwoCropSeparate,
)
from da_hos.modeling.meta_arch.ts_ensemble import EnsembleTSModel
from da_hos.checkpoint.detection_checkpoint import DetectionTSCheckpointer
from da_hos.solver.build import build_lr_scheduler
from da_hos.evaluation import PascalVOCDetectionEvaluator, COCOEvaluator
...
class ATeacherTrainer(DefaultTrainer):
    ...
    def threshold_bbox(self, proposal_bbox_inst, thres=0.7, proposal_type="roih"):
        if proposal_type == "rpn":
            ...
            new_proposal_inst.gt_handsides = torch.argmax(
                proposal_bbox_inst.pred_handsides[valid_map], dim=1
            )
            new_proposal_inst.gt_contacts = torch.argmax(
                proposal_bbox_inst.pred_contacts[valid_map], dim=1
            )
            new_proposal_inst.gt_offsets = proposal_bbox_inst.pred_offsets[valid_map]
