...
def hos_inference(
    ...
    """
    Call `hos_inference_single_image` for all images.
...
def hos_inference_single_image(
    boxes,
    scores,
    handsides,
    contacts,
    ...
    Args:
        Same as `hos_inference`, but with boxes, scores, and image shapes
        per image.

    Returns:
        Same as `hos_inference`, but for only one image.
    """
...
class HOSFastRCNNOutputLayers(nn.Module):
    ...
    def losses(self, predictions, proposals):
        ...
        gt_handsides = cat([p.gt_handsides for p in proposals], dim=0) if len(proposals) else torch.empty(0)

        # parse contact classification outputs
        gt_contacts = cat([p.gt_contacts for p in proposals], dim=0) if len(proposals) else torch.empty(0)
        ...
        losses["loss_box_reg"] = self.box_reg_loss(proposal_boxes, gt_boxes, proposal_deltas, gt_classes)

        losses["loss_handside"] = self.handside_clf_loss(handsides, gt_handsides, hand_mask)
        losses["loss_contact"] = self.contact_clf_loss(contacts, gt_contacts, hand_mask * contact_mask)
        # losses["loss_contact"] = self.contact_clf_loss(contacts, gt_contacts, hand_mask)
        losses["loss_offset"] = self.mse_loss(
    ...
    def handside_clf_loss(self, pred_handsides, gt_handsides, hand_mask):
        ...
        if not isinstance(loss_handside, torch.Tensor):
            loss_handside = cross_entropy(torch.tensor([]).to(hand_mask.device), torch.tensor([]).to(hand_mask.device))
        return loss_handside / max(hand_mask.shape[0], 1.0)

    def contact_clf_loss(self, pred_contacts, gt_contacts, contact_mask):
        loss_contact = 0
        gt_contacts[contact_mask.squeeze(1) == False] = -1
        loss_contact = cross_entropy(pred_contacts, gt_contacts, ignore_index=-1, reduction="none")
...
class HOSMultiBackBoneFastRCNNOutputLayers(HOSFastRCNNOutputLayers):
    def __init__(self, cfg, *args, **kwargs):
        super().__init__(cfg, *args, **kwargs)
        self.contact_loss_weight = cfg.MODEL.get("CONTACT_LOSS_WEIGHT", 1)

    def forward(self, x, x_contact):
        if x.dim() > 2:
            x = torch.flatten(x, start_dim=1)
        scores = self.cls_score(x)
        proposal_deltas = self.bbox_pred(x)
        ...
        # hos added
        handsides = self.handside_pred(x)
        contacts = self.contact_pred(x_contact)
        offsets = self.offset_pred(x)
    ...
    def contact_clf_loss(self, pred_contacts, gt_contacts, contact_mask):
        ...
        gt_contacts[contact_mask.squeeze(1) == False] = -1
        loss_contact = cross_entropy(pred_contacts, gt_contacts, ignore_index=-1, reduction="none")
        loss_contact = loss_contact.mean() * self.contact_loss_weight

        return loss_contact


class HOSMultiBackBoneFreezedFastRCNNOutputLayers(HOSFastRCNNOutputLayers):
    def __init__(self, cfg, *args, **kwargs):
        super().__init__(cfg, *args, **kwargs)
        self.contact_loss_weight = cfg.MODEL.get("CONTACT_LOSS_WEIGHT", 1)
