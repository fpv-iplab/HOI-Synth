...
from da_hos.data.detection_utils import build_strong_augmentation
...
def hos_annotations_to_instances(annos, image_size, mask_format="polygon"):
    boxes = (
        np.stack(
            [
...
class DAHOSDatasetMapperTwoCropSeparate(DatasetMapper):
    def __init__(self, cfg, is_train=True):
        ...
        if cfg.INPUT.CROP.ENABLED and is_train:
            ...
            logging.getLogger(__name__).info("Cropping used in training: " + str(self.augmentation[0]))
        ...
        self.keypoint_hflip_indices = utils.create_keypoint_hflip_indices(cfg.DATASETS.TRAIN) if self.keypoint_on and is_train else None
        ...
        self.depth_paths_labeled_train = cfg.DATASETS.get("TRAIN_DEPTHS_PATH", None)
        self.depth_paths_test = cfg.DATASETS.get("TEST_DEPTHS_PATH", None)
        self.depth_paths_unlabeled = cfg.DATASETS.get("TRAIN_UNLABEL_DEPTHS_PATH", None)
        self.depth_paths_weaklabeled = cfg.DATASETS.get("TRAIN_WEAK_DEPTHS_PATH", None)
        
        cfg.DATASETS.TRAIN_UNLABEL_DEPTHS_PATH if self.depth_enabled else None  

        self.modality = cfg.MODEL.get("MODALITY", "RGB")

        self.sam_enabled = cfg.MODEL.get("SAM_ENABLED", False)
    ...
    def __call__(self, dataset_dict):
        ...
        sem_seg_gt = utils.read_image(dataset_dict.pop("sem_seg_file_name"), "L").squeeze(2)   if "sem_seg_file_name" in dataset_dict else None
        ...
        if self.depth_enabled:
            ...
            if os.path.exists(os.path.join(self.depth_paths_labeled_train, dataset_dict["file_name"].split("/")[-1].replace(".jpg", "_depth.jpg"))):
                depth_path = os.path.join(self.depth_paths_labeled_train, dataset_dict["file_name"].split("/")[-1].replace(".jpg", "_depth.jpg"))
            elif os.path.exists(os.path.join(self.depth_paths_unlabeled, dataset_dict["file_name"].split("/")[-1].replace(".jpg", "_depth.jpg"))):
                depth_path = os.path.join(self.depth_paths_unlabeled, dataset_dict["file_name"].split("/")[-1].replace(".jpg", "_depth.jpg"))
            else:
                depth_path = os.path.join(self.depth_paths_weaklabeled, dataset_dict["file_name"].split("/")[-1].replace(".jpg", "_depth.jpg"))

            depth = utils.read_image(depth_path, self.img_format)
            depth = cv2.applyColorMap(depth, cv2.COLORMAP_INFERNO) if self.modality == "DEPTH_JET" else depth
            depth = cv2.resize(depth, (image_shape[1], image_shape[0]))
        ...
        if "annotations" in dataset_dict:
            ...
            instances = hos_annotations_to_instances(
        ...
        if self.depth_enabled:
            ...
            dataset_dict_key["depth"] = torch.as_tensor(np.ascontiguousarray(depth.transpose(2, 0, 1)))
    ...
    def inference(self, dataset_dict):
        ...
        if self.depth_enabled:
            depth_path = os.path.join(self.depth_paths_test, dataset_dict["file_name"].split("/")[-1].replace(".jpg", "_depth.jpg"))
            depth = utils.read_image(depth_path, self.img_format)
            depth = cv2.applyColorMap(depth, cv2.COLORMAP_INFERNO) if self.modality == "DEPTH_JET" else depth
            depth = cv2.resize(depth, (image_shape[1], image_shape[0]))
            dataset_dict["depth"] = torch.as_tensor(np.ascontiguousarray(depth.transpose(2, 0, 1)))
