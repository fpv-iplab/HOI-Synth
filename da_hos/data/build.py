...
from da_hos.data.common import (
    AspectRatioGroupedDatasetTwoCrop,
    AspectRatioGroupedSemiSupDatasetTwoCrop,
    AspectRatioGroupedSemiSupDatasetTwoCropMulti,
)
...
def build_detection_semisup_train_loader_two_crops_multi(cfg, mapper=None):
    ...
    weak_label_dicts = get_detection_dataset_dicts(
        cfg.DATASETS.TRAIN_WEAK_LABEL,
        filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
        min_keypoints=(
            cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
            if cfg.MODEL.KEYPOINT_ON
    ...
    # exclude the labeled set from unlabeled dataset
    unlabel_dataset = DatasetFromList(unlabel_dicts, copy=False)
    weak_labeled_dataset = DatasetFromList(weak_label_dicts, copy=False)
    ...
    label_dataset = MapDataset(label_dataset, mapper)
    unlabel_dataset = MapDataset(unlabel_dataset, mapper)
    weak_labeled_dataset = MapDataset(weak_labeled_dataset, mapper)
    ...
    if sampler_name == "TrainingSampler":
        label_sampler = TrainingSampler(len(label_dataset))
        unlabel_sampler = TrainingSampler(len(unlabel_dataset))
        weak_labeled_sampler = TrainingSampler(len(weak_labeled_dataset))
    elif sampler_name == "RepeatFactorTrainingSampler":
        raise NotImplementedError("{} not yet supported.".format(sampler_name))
    else:
        raise ValueError("Unknown training sampler: {}".format(sampler_name))

    return build_semisup_batch_data_loader_two_crop_multi(
        (label_dataset, unlabel_dataset, weak_labeled_dataset),
        (label_sampler, unlabel_sampler, weak_labeled_sampler),
        cfg.SOLVER.IMG_PER_BATCH_LABEL,
        cfg.SOLVER.IMG_PER_BATCH_UNLABEL,
        cfg.SOLVER.IMG_PER_BATCH_WEAK,
        aspect_ratio_grouping=cfg.DATALOADER.ASPECT_RATIO_GROUPING,
        num_workers=cfg.DATALOADER.NUM_WORKERS,
    )
...
# batch data loader
def build_semisup_batch_data_loader_two_crop_multi(
    dataset,
    sampler,
    total_batch_size_label,
    total_batch_size_unlabel,
    total_batch_size_weak_label,
    *,
    aspect_ratio_grouping=False,
    num_workers=0
):
    ...
    batch_size_label = total_batch_size_label // world_size
    batch_size_unlabel = total_batch_size_unlabel // world_size
    batch_size_weak_label = total_batch_size_weak_label // world_size

    label_dataset, unlabel_dataset, weak_label_dataset = dataset
    label_sampler, unlabel_sampler, weak_label_sampler = sampler
    ...
    weak_label_data_loader = torch.utils.data.DataLoader(
        weak_label_dataset,
        sampler=weak_label_sampler,
        num_workers=num_workers,
        batch_sampler=None,
        collate_fn=operator.itemgetter(0),  # don't batch, but yield individual elements
        worker_init_fn=worker_init_reset_seed,
    ...
    return AspectRatioGroupedSemiSupDatasetTwoCropMulti(
        (label_data_loader, unlabel_data_loader, weak_label_data_loader),
        (batch_size_label, batch_size_unlabel, batch_size_weak_label),
    )
