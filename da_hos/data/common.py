...
class AspectRatioGroupedSemiSupDatasetTwoCropMulti(AspectRatioGroupedDataset):
    def __init__(self, dataset, batch_size):
        self.label_dataset, self.unlabel_dataset, self.weak_label_dataset = dataset
        self.batch_size_label = batch_size[0]
        self.batch_size_unlabel = batch_size[1]
        self.batch_size_weak_label = batch_size[2]

        self._label_buckets = [[] for _ in range(2)]
        self._label_buckets_key = [[] for _ in range(2)]
        self._unlabel_buckets = [[] for _ in range(2)]
        self._unlabel_buckets_key = [[] for _ in range(2)]
        self._weak_label_buckets = [[] for _ in range(2)]
        self._weak_label_buckets_key = [[] for _ in range(2)]

        # Hard-coded two aspect ratio groups: w > h and w < h.
        # Can add support for more aspect ratio groups, but doesn't seem useful

    def __iter__(self):
        label_bucket, unlabel_bucket, weak_label_bucket = [], [], []
        for d_label, d_unlabel, d_weak_label in zip(self.label_dataset, self.unlabel_dataset, self.weak_label_dataset):
            # d is a tuple with len = 2
            # It's two images (same size) from the same image instance
            # d[0] is with strong augmentation, d[1] is with weak augmentation
            ...
            if len(weak_label_bucket) != self.batch_size_weak_label:
                w, h = d_weak_label[0]["width"], d_weak_label[0]["height"]
                weak_label_bucket_id = 0 if w > h else 1
                weak_label_bucket = self._weak_label_buckets[weak_label_bucket_id]
                weak_label_bucket.append(d_weak_label[0])
                weak_label_buckets_key = self._weak_label_buckets_key[weak_label_bucket_id]
                weak_label_buckets_key.append(d_weak_label[1])

            # yield the batch of data until all buckets are full
            if (
                len(label_bucket) == self.batch_size_label
                and len(unlabel_bucket) == self.batch_size_unlabel
                and len(weak_label_bucket) == self.batch_size_weak_label
            ):
                # label_strong, label_weak, unlabed_strong, unlabled_weak
                yield (
                    label_bucket[:],
                    ...
                    unlabel_buckets_key[:],
                    weak_label_bucket[:],
                    weak_label_buckets_key[:],
                )
                del label_bucket[:]
                del label_buckets_key[:]
                del unlabel_bucket[:]
                del unlabel_buckets_key[:]
                del weak_label_bucket[:]
                del weak_label_buckets_key[:]