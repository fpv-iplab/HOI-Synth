...
_RAW_CITYSCAPES_SPLITS = {
    ...
    "cityscapes_foggy_test": (
        "cityscapes_foggy/leftImg8bit/test/",
        "cityscapes_foggy/gtFine/test/",
    ),
}
...
# ==== CUSTOM ===========
path_unity = "/SSD/rleonardi/data/Unity_perception/unity_qz43gceebh/"
path_visor = "/home/rleonardi/VISOR-HOS/datasets/epick_visor_coco_hos/"
path_unity_visor = (
    "/SSD/rleonardi/data/Unity_perception/unity_qz43gceebh/unity_visor_coco/"
)


# ==== Predefined splits for unity ===========
def register_unity(root):
    SPLITS = [
        ("unity_train", "coco", "train"),
        ("unity_val", "coco", "val"),
    ]
    for name, dirname, split in SPLITS:
        register_coco_instances(
            name=name,
            metadata={},
            json_file=os.path.join(root, dirname, f"annotations/{split}.json"),
            image_root=os.path.join(root, dirname, "images"),
        )
        MetadataCatalog.get(name).evaluator_type = "coco"


# ==== Predefined splits for visor ===========
def register_visor(root):
    register_coco_unlabel_instances(
        "visor_train",
        {},
        os.path.join(root, f"annotations/train.json"),
        os.path.join(root, "train"),
    )
    register_coco_instances(
        name="visor_val",
        metadata={},
        json_file=os.path.join(root, f"annotations/val.json"),
        image_root=os.path.join(root, "val"),
    )
    MetadataCatalog.get("visor_val").evaluator_type = "coco"


# ==== Predefined splits for combo unity_visor ===========
def register_unity_visor(root):
    SPLITS = [
        ("unity_visor10_train", "train_10"),
        ("unity_visor25_train", "train_25"),
        ("unity_visor50_train", "train_50"),
        ("unity_visor100_train", "train_100"),
    ]
    for name, split in SPLITS:
        register_coco_instances(
            name=name,
            metadata={},
            json_file=os.path.join(root, f"annotations/{split}.json"),
