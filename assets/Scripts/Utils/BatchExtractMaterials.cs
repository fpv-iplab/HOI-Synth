...
public class BatchExtractMaterials : EditorWindow
{
    ...
    private class ExtractData
    {
        ...
        public ExtractData(GameObject model)
        {
            this.model = model;
        }
    }
    ...
    private class RemapAllPopup : EditorWindow
    {
        ...
        public static void ShowAt(
            Rect buttonRect,
            Vector2 size,
            System.Action<List<Material>, Material, bool> onRemapConfirmed
        )
        {
            buttonRect.position = GUIUtility.GUIToScreenPoint(buttonRect.position);

            remapAllPopup = GetWindow<RemapAllPopup>(true);
            remapAllPopup.position = new Rect(
                buttonRect.position
                    + new Vector2((buttonRect.width - size.x) * 0.5f, buttonRect.height),
                size
            );
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField(
                "This will find all materials that point to 'Remap From' and remap them to 'Remap To'. If 'Remap From' is empty, all materials will be remapped to 'Remap To'.",
            ...
            remapTo =
                EditorGUILayout.ObjectField("Remap To", remapTo, typeof(Material), false)
                as Material;

            EditorGUI.BeginChangeCheck();
            skipIgnoredMaterials = EditorGUILayout.Toggle(
                "Skip Ignored Materials",
                skipIgnoredMaterials
            );
            ...
            if (GUILayout.Button("Apply"))
            {
                if (remapTo && onRemapConfirmed != null)
                {
                    ...
                    onRemapConfirmed(
                        remapFromIsFilled ? remapFrom : null,
                        remapTo,
                        skipIgnoredMaterials
                    );
                }
    ...
    private void OnGUI()
    {
        ...
        {
            GUI.enabled =
                modelsToProcessListIsFilled
                && !string.IsNullOrEmpty(materialsFolder)
                && materialsFolder.StartsWith("Assets");
    ...
    private void DrawDestinationPathField()
    {
        ...
        )
        {
            ...
            if (ev.type == EventType.DragPerform)
            {
                ...
                {
                    if (
                        !string.IsNullOrEmpty(draggedFiles[i])
                        && AssetDatabase.IsValidFolder(draggedFiles[i])
                    )
                    {
        ...
        if (GUILayout.Button("o", GL_WIDTH_25))
        {
            ...
            if (!string.IsNullOrEmpty(selectedPath))
            {
                ...
                if (relativePathIndex > 0)
                    materialsFolder = selectedPath.Substring(
                        relativePathIndex,
                        selectedPath.Length - relativePathIndex - 1
                    );
            }
    ...
    private void DrawModelsToProcessList()
    {
        ...
        )
        {
            ...
            if (ev.type == EventType.DragPerform)
            {
                ...
                {
                    if (
                        !(draggedObjects[i] as GameObject)
                        || PrefabUtility.GetPrefabAssetType(draggedObjects[i])
        ...
        for (int i = 0; i < modelData.Count; i++)
        {
            ...
            GameObject newObject =
                EditorGUILayout.ObjectField(GUIContent.none, prevObject, typeof(GameObject), false)
                as GameObject;
    ...
    private void DrawMaterialRemapConditionsField()
    {
        ...
        remappedMaterialNamesMustMatch = EditorGUILayout.ToggleLeft(
            "Material names must match",
            remappedMaterialNamesMustMatch
        );
        remappedMaterialPropertiesMustMatch = EditorGUILayout.ToggleLeft(
            "Material properties must match",
            remappedMaterialPropertiesMustMatch
        );
        dontRemapExtractedMaterials = EditorGUILayout.ToggleLeft(
            "Don't remap already extracted materials",
            dontRemapExtractedMaterials
        );
        dontRemapMaterialsAcrossDifferentModels = EditorGUILayout.ToggleLeft(
            "Don't remap Model A's materials to Model B (i.e. different models won't share the same materials)",
            dontRemapMaterialsAcrossDifferentModels
        );
    ...
    private void DrawMaterialRemapList()
    {
        ...
        {
            RemapAllPopup.ShowAt(
                remapAllButtonRect,
                new Vector2(325f, 250f),
                (List<Material> remapFrom, Material remapTo, bool skipIgnoredMaterials) =>
                {
                    for (int i = 0; i < modelData.Count; i++)
                    {
                            ...
                            switch (data.materialExtractModes[j])
                            {
                                case ExtractMode.Extract:
                                {
                                    if (
                                        remapFrom == null
                                        || (
                                            data.originalMaterials[j]
                                            && remapFrom.Contains(data.originalMaterials[j])
                                        )
                                    )
                                    {
                                ...
                                case ExtractMode.Remap:
                                {
                                    if (
                                        remapFrom == null
                                        || (
                                            data.remappedMaterials[j]
                                            && remapFrom.Contains(data.remappedMaterials[j])
                                        )
                                    )
                                        data.remappedMaterials[j] = remapTo;
        ...
        for (int i = 0; i < modelData.Count; i++)
        {
            ...
            for (int j = 0; j < data.originalMaterials.Count; j++)
            {
                ...
                data.materialExtractModes[j] = (ExtractMode)
                    EditorGUILayout.EnumPopup(
                        GUIContent.none,
                        data.materialExtractModes[j],
                        GL_WIDTH_75
                    );
                if (data.materialExtractModes[j] == ExtractMode.Remap)
                {
                    ...
                    if (
                        ...
                        && (
                            !data.remappedMaterials[j]
                            || data.remappedMaterials[j] == data.originalMaterials[j]
                        )
                    )
    ...
    private void CalculateRemappedMaterials()
    {
        ...
        Dictionary<int, HashSet<Material>> duplicateMaterialsLookup = new Dictionary<
            int,
            HashSet<Material>
        >(modelData.Count * 8);
        ...
        {
            string[] existingMaterialPaths = Directory.GetFiles(
                materialsFolder,
                "*.mat",
            ...
            {
                Material material =
                    AssetDatabase.LoadMainAssetAtPath(existingMaterialPaths[i]) as Material;
                if (material)
        ...
        for (int i = 0; i < modelData.Count; i++)
        {
            ...
            {
                Debug.LogWarning(
                    "Couldn't get ModelImporter from asset: "
                        + AssetDatabase.GetAssetPath(data.model),
                    data.model
                );
                modelData.RemoveAt(i--);
                continue;
            ...
            using (SerializedObject so = new SerializedObject(modelImporter))
            {
                ...
                for (int materialIndex = 0; materialIndex < materials.arraySize; materialIndex++)
                {
                    ...
                    for (
                        int externalObjectIndex = 0;
                        externalObjectIndex < externalObjects.arraySize;
                        externalObjectIndex++
                    )
                    {
                        SerializedProperty pair = externalObjects.GetArrayElementAtIndex(
                            externalObjectIndex
                        );
                        ...
                        if (
                            externalType == type
                            && externalName == name
                            && (pair = pair.FindPropertyRelative("second")) != null
                        )
                        {
...
#if UNITY_2019_1_OR_NEWER
                        HashSet<Material> duplicateMaterials = GetMaterialsWithCRC(
                            duplicateMaterialsLookup,
                            material
                        );
                        ...
                        if (!materialAlreadyExtracted || !dontRemapExtractedMaterials)
                        {
                            if (remappedMaterialPropertiesMustMatch)
                            {
                                foreach (Material _material in duplicateMaterials)
                                {
                                    if (
                                        _material.name == name
                                        || (!remappedMaterial && !remappedMaterialNamesMustMatch)
                                    )
...
#endif
                        {
                            data.materialExtractModes.Add(
                                materialAlreadyExtracted ? ExtractMode.Ignore : ExtractMode.Extract
                            );
                            data.remappedMaterials.Add(null);
...
#if UNITY_2019_1_OR_NEWER
    private HashSet<Material> GetMaterialsWithCRC(
        Dictionary<int, HashSet<Material>> lookup,
        Material material
    )
    {
    ...
    private void ExtractMaterials()
    {
        ...
        Dictionary<Material, Material> extractedMaterials = new Dictionary<Material, Material>(
            modelData.Count * 8
        );
        ...
        for (int i = 0; i < modelData.Count; i++)
        {
            ...
            AssetImporter modelImporter = AssetImporter.GetAtPath(
                AssetDatabase.GetAssetPath(data.model)
            );
            ...
            using (SerializedObject so = new SerializedObject(modelImporter))
            {
                ...
                for (int materialIndex = 0; materialIndex < materials.arraySize; materialIndex++)
                {
                    ...
                    int j =
                        (
                            materialIndex < data.materialNames.Count
                            && data.materialNames[materialIndex] == name
                        )
                            ? materialIndex
                            : data.materialNames.IndexOf(name);
                    if (j < 0)
                    {
                        ...
                        Debug.LogWarning(
                            data.model.name
                                + "."
                                + name
                                + " material has no matching data, skipped",
                            data.model
                        );
                        continue;
                    }
                    ...
                    switch (data.materialExtractModes[j])
                    {
                        case ExtractMode.Extract:
                        {
                            if (
                                data.originalMaterials[j]
                                && !AssetDatabase.IsMainAsset(data.originalMaterials[j])
                            )
                                ...
                                Debug.LogWarning(
                                    data.model.name
                                        + "."
                                        + name
                                        + " isn't extracted because either the material doesn't exist or it is already extracted",
                                    data.model
                                );
                        ...
                        case ExtractMode.Remap:
                        {
                            if (
                                data.remappedMaterials[j]
                                && (
                                    data.originalMaterials[j] != data.remappedMaterials[j]
                                    || !AssetDatabase.IsMainAsset(data.remappedMaterials[j])
                                )
                            )
                                ...
                                Debug.LogWarning(
                                    data.model.name
                                        + "."
                                        + name
                                        + " isn't remapped because either the material doesn't exist or it is already extracted",
                                    data.model
                                );
                    ...
                    else if (!AssetDatabase.IsMainAsset(targetMaterial))
                    {
                        ...
                        {
                            extractedMaterials[targetMaterial] = extractedMaterial = new Material(
                                targetMaterial
                            );
                            AssetDatabase.CreateAsset(
                                extractedMaterial,
                                AssetDatabase.GenerateUniqueAssetPath(
                                    materialsFolder + "/" + targetMaterial.name + ".mat"
                                )
                            );
                        }
                    ...
                    {
                        SerializedProperty currentSerializedProperty =
                            externalObjects.GetArrayElementAtIndex(externalObjects.arraySize++);
                        currentSerializedProperty.FindPropertyRelative("first.name").stringValue =
                            name;
                        currentSerializedProperty.FindPropertyRelative("first.type").stringValue =
                            type;
                        currentSerializedProperty
                            .FindPropertyRelative("first.assembly")
                            .stringValue = id.FindPropertyRelative("assembly").stringValue;
                        currentSerializedProperty
                            .FindPropertyRelative("second")
                            .objectReferenceValue = targetMaterial;
                    }
                    else
